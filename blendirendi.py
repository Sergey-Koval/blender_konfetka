...
#reqired librarys:
'''
psutil pillow bottle tornado py-cpuinfo requests
...
def index():
    ...
    try:
        ...
        try:
            shutil.rmtree("data" + os.path.sep + "%d" % job_id)
        except:
            traceback.print_exc()
            pass
...
def index():
    ...
    try:
        ...
        if delete_image_file:
            ...
            frame_nr = data[1]
            try:
                os.remove("data" + os.path.sep + "%d" + os.path.sep + "%04d.png" % (job_id, frame_nr))
            except Exception:
                pass #might fail if the image is not present
            try:
                os.remove("data" + os.path.sep + "%d" + os.path.sep + "%04d_thumb.png" % (job_id, frame_nr))
            except Exception:
                pass #might fail if the image is not present
        db.execute("commit transaction")
        return json.dumps({})
...
def index():
    ...
    try:
        ...
        os.makedirs("data" + os.path.sep + "%d" % (job_id,))
        print("save uploaded new job file")
        upload.save("data" + os.path.sep + str(job_id) + os.path.sep + upload.filename)
...
def index():
    ...
    try:
        ...
        if success:
            ...
            job_id = data[0]
            frame_nr = data[1]
            filename_norm = "data" + os.path.sep + str(job_id) + os.path.sep + "%04d.png" % (frame_nr)
            filename_thumb = "data" + os.path.sep + str(job_id) + os.path.sep + "%04d_thumb.png" % (frame_nr)
...
def index():
    ...
    try:
        ...
        print(filename)
        return static_file("data" + os.path.sep + str(job_id) + os.path.sep + filename, root='.')
    except Exception as e:
        traceback.print_exc()
        return resp_exception(str(e))
...
def index():
    ...
    try:
        ...
        return static_file("data" + os.path.sep + "%d" + os.path.sep + "%04d.png" % (job_id, frame_no), root='.')
    except Exception as e:
        traceback.print_exc()
        return resp_exception(str(e))
...
def index():
    ...
    try:
        ...
        return static_file("data" + os.path.sep + str(job_id) + os.path.sep + "%04d_thumb.png" % (frame_no), root='.')
    except Exception as e:
        traceback.print_exc()
        return resp_exception(str(e))
...
def cache_blend(job_id, job_name):
    local_directory = "cache" + os.path.sep + "%d" % job_id
    local_blendname = "cache" + os.path.sep + str(job_id) + os.path.sep + job_name
...
def do_cache_cleanup_if_need():
    ...
    if now - last_cache_cleanup > 1000*60*60*3:
        searchpath = "cache"
        print("1", searchpath)
        print("2", os.path.join(searchpath, "test"))
    ...
    while True:
        try:
            ...
            if "exception" in respjson:
                ...
                path_to_blend = "cache" + os.path.sep + str(job_id) + os.path.sep + job_name
                blender_output_path = os.path.abspath("cache" + os.path.sep + str(job_id) + os.path.sep) + os.path.sep
